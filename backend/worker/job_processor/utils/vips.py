...
class ImageProcessingLibrary:
    ...
    def _compress_image_sync(
        ...
        try:
            ...
            logging.debug(
                f"[Timing {input_path}] Load image: {time.monotonic() - start:.2f}s"
            )
            ...
            logging.info(f"[Timing {input_path}] EXIF: {time.monotonic() - start:.2f}s")
            ...
            logging.info(
                f"[Timing {input_path}] Resize: {time.monotonic() - start:.2f}s"
            )
            ...
            logging.info(
                f"[Timing {input_path}] write_to_file: {time.monotonic() - start:.2f}s"
            )
